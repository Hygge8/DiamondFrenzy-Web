...
class StorageUtils {
     ...
     * @returns {any} 存储的值
     */
    static get(key, defaultValue = null, storageType = this.STORAGE_TYPE.LOCAL, prefix = this.DEFAULT_PREFIX) {
        try {
            const storage = this._getStorage(storageType);
     ...
     */
    static setWithExpiry(key, value, expiryHours = 24, storageType = this.STORAGE_TYPE.LOCAL, prefix = this.DEFAULT_PREFIX) {
        try {
            const storage = this._getStorage(storageType);
            if (!storage) return false;
     ...
     * @returns {any} 存储的值
     */
    static getWithExpiry(key, defaultValue = null, storageType = this.STORAGE_TYPE.LOCAL, prefix = this.DEFAULT_PREFIX) {
        try {
            const storage = this._getStorage(storageType);
            if (!storage) return defaultValue;
     ...
     */
    static setCompressed(key, value, storageType = this.STORAGE_TYPE.LOCAL, prefix = this.DEFAULT_PREFIX) {
        try {
            // 简单的数据压缩（移除不必要的空白字符）
            const compressed = typeof value === 'string' 
     ...
     */
    static getMultiple(keys, defaultValues = {}, storageType = this.STORAGE_TYPE.LOCAL, prefix = this.DEFAULT_PREFIX) {
        const result = {};
        keys.forEach(key => {
     ...
     */
    static import(data, overwrite = false, storageType = this.STORAGE_TYPE.LOCAL, prefix = this.DEFAULT_PREFIX) {
        try {
            const storage = this._getStorage(storageType);
            if (!storage) return false;
