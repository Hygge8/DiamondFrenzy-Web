...
class LevelManager {
        ...
        return [
            {
                ...
                enemies: [
                    { x: 350, y: 400, type: 'red_snake' }
                ],
                items: [
                    { x: 150, y: 450, type: 'compass' }
                ],
    ...
    getNearbyObstacles(player, range) {
        return this.obstacles.filter(obstacle => {
            return obstacle.isActive && 
                   !obstacle.isDead && 
                   player.getDistance(obstacle) <= range;
        });
    }
    ...
    getNearbyEnemies(player, range) {
        return this.enemies.filter(enemy => {
            return enemy.isActive && 
                   !enemy.isDead && 
                   player.getDistance(enemy) <= range;
        });
    }
    ...
    _renderDebugInfo(ctx) {
        ...
        ctx.fillText(`钻石: ${this.player.diamondsCollected}/${this.currentLevel.targetDiamonds}`, 10, y);
        y += lineHeight;
